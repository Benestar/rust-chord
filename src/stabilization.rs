...
impl Stabilization {
       ...
       -> ::Result<Self>
    {
        ...
        let successor = procedures.find_peer(current_addr.identifier(), boot_addr)?;
    ...
    pub fn stabilize(&mut self) -> ::Result<()> {
        ...
        let new_successor = self.procedures.get_predecessor(*self.routing.successor)?;
    ...
    pub fn update_fingers(&mut self) -> ::Result<()> {
        ...
        for (i, finger) in self.routing.finger_table.iter_mut().enumerate() {
            ...
            let peer = self.procedures.find_peer(identifier, *self.routing.successor)?;
